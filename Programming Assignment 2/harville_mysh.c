...
char *search_in_path(char *path, char *filename);
...
void execute_command(char **args) {
    ...
    // If the command is not built-in, attempt to execute it as an external program.
...
void execute_external_command(char **args) {
    ...
    if (pid == 0) {
        ...
        if (strchr(args[0], '/') == NULL) {
            // Simple program name, search in PATH
            char *path = getenv("PATH");
            char *exec_path = search_in_path(path, args[0]);
            if (exec_path != NULL) {
        ...
        exit(EXIT_FAILURE); // execv only returns if there is an error
    } else if (pid < 0) {
        // Error forking
        perror("fork");
    } else {
...
char *search_in_path(char *path, char *filename) {
    ...
    while (token != NULL) {
        char *fullpath = malloc(strlen(token) + strlen(filename) + 2);
        sprintf(fullpath, "%s/%s", token, filename);
        if (access(fullpath, X_OK) == 0) {
            return fullpath; // Found executable
        }
        free(fullpath);
        token = strtok(NULL, ":");
    }
